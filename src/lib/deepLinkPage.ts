...
function safeJson(value: string): string {
  // JSON.stringify escapes JS string chars but NOT </script> which closes the block.
...
export function buildDeepLinkPage({ id, type, ogTitle, ogDescription }: DeepLinkPageOptions): string {
  const safeId = escapeHtml(id);
  const appUrl = `quickcourt://view-${type}/${safeId}`;
  const webUrl = `https://quickcourt.co/view-${type}/${safeId}`;
...
<body>
  ...
  <script>
    (function(){
      var appUrl = ${safeJson(`quickcourt://view-${type}/${id}`)};
      ...
      // Always show store links — on desktop both are shown as a fallback
      if(isIOS || !isMobile) show('btn-ios');
      if(isAndroid || !isMobile) show('btn-android');
